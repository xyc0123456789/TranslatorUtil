            ...
            for cur_page in cur_pdf:
                ...
                for num in range(len(blks)):  # loop through the blocks
                    ...
                    if blks[num][-1] == 1:
                        ...
                        try:
                            path_img = os.path.join(tempDir, '图片{}.png'.format(imgcount))  # 当前页面第几个图片的位置
                    ...
                    # 两个块y轴距离远的的时候
                        ...
                        if flag == 1:
                            ...
                            if begin[2] > end[2]:  # 如果起始点的右下角x坐标小于结束点的右下角x坐标
                                new_page.insert_textbox(fitz.Rect(begin[0], begin[1], begin[2], end[3]), res, fontname="song",
                                                        fontfile=self.frontFilePath,
                                                        fontsize=fonts, align=text_pos)
                            else:
                                new_page.insert_textbox(fitz.Rect(begin[0], begin[1], end[2], end[3]), res, fontname="song",
                                                        fontfile=self.frontFilePath,
                                                        fontsize=fonts, align=text_pos)
                            flag = 0
                        else:
                            ...
                            # 标记在这里之后的都是参考文献
                                ...
                                if reference_flag == 1:
                                    ...
                                    new_page.insert_textbox(r, res, fontname="song", fontfile=self.frontFilePath, fontsize=fonts, align=text_pos)
                        # 记录起始矩形坐标
                        begin = blks[num + 1][:4]
                        try:
                            content = blks[num + 1][4].replace("\n", " ")
